...
pipeline {
  ...
  stages {
    stage('Checkout') {
      steps {
        sh 'echo passed'
        //git branch: 'main', url: 'https://github.com/Hydra2206/Jenkins-Zero-To-Hero.git'
      }
    }
    ...
    stage('Static Code Analysis') {
      environment {
        SONAR_URL = "http://13.205.168.87:9000/"
      }
      steps {
        withCredentials([string(credentialsId: 'sonarqube', variable: 'SONAR_AUTH_TOKEN')]) {
    ...
    stage('Build and Push Docker Image') {
      environment {
        DOCKER_IMAGE = "mittu7/ultimate-cicd:${BUILD_NUMBER}"
        ...
        steps {
            withCredentials([string(credentialsId: 'github', variable: 'GITHUB_TOKEN')]) {
                sh '''
                    pwd
                    ls -la
                    git config --global user.email "mittu270318@gmail.com"
                    git config --global user.name "Hydra2206"
                    BUILD_NUMBER=${BUILD_NUMBER}
                    sed -i "s/replaceImageTag/${BUILD_NUMBER}/g" java-maven-sonar-argocd-helm-k8s/spring-boot-app-manifests/deployment.yml
                    git config --global --add safe.directory /var/lib/jenkins/workspace/java-maven-pipeline@2
